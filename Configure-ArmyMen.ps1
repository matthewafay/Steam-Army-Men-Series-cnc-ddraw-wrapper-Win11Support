...
function Set-GameResolution {
    ...
    try {
        ...
        # Determine if using dgVoodoo2 (Army Men II and RTS) or cnc-ddraw (others)
        $useDgVoodoo = $GameInfo -and ($GameInfo.Name -eq "Army Men II" -or $GameInfo.Name -eq "Army Men RTS")
        
        if ($useDgVoodoo) {
            # dgVoodoo2 configuration - fullscreen mode for proper vehicle movement
            $configContent['ScreenWidth'] = '1024'
            $configContent['ScreenHeight'] = '768'
            $configContent['Windowed'] = '0'
            $configContent['FullScreen'] = '1'
            $configContent['ColorDepth'] = '16'
            $configContent['DirectDraw'] = '1'
            $configContent['Direct3D'] = '1'
            $configContent['Hardware3D'] = '1'
        } else {
            # cnc-ddraw configuration - windowed mode with upscaling
...
showfps=true
no_compat_warning=true
; Vehicle movement fix settings
fixchilds=true
fixnotresponding=true
fixmousehook=true
; Performance settings for smooth vehicle movement
limiter=false
minfps=30
"@
        }
...
function Install-DgVoodoo2 {
    ...
    try {
        Write-Status -Message "Installing dgVoodoo2 for graphics compatibility..." -Type "Info"
        ...
        # Create dgVoodoo2 configuration based on game
        $gameName = if ($GameInfo) { $GameInfo.Name } else { "Army Men" }
        
        # Army Men II needs fullscreen mode for proper vehicle movement
        $fullScreenMode = if ($gameName -eq "Army Men II") { "true" } else { "false" }
        $appControlled = if ($gameName -eq "Army Men II") { "true" } else { "false" }
        
        $dgVoodooConfig = @"
;==========================================================================
; dgVoodoo2 config for $gameName - Generated by Configure-ArmyMen.ps1
;==========================================================================
Version                              = 0x286
...
Resolution                           = unforced
Antialiasing                         = appdriven
AppControlledScreenMode              = $appControlled
...
function Invoke-ArmyMenConfiguration {
    ...
    if ($script:ConfigState.SelectedGame.Name -eq "Army Men RTS" -or $script:ConfigState.SelectedGame.Name -eq "Army Men II") {
        # Use dgVoodoo2 for Army Men RTS and Army Men II (fixes vehicle movement issues)
        Write-Status -Message "Installing dgVoodoo2 graphics compatibility layer..." -Type "Info"
        
        try {
            $dgVoodooResult = Install-DgVoodoo2 -GamePath $script:ConfigState.GamePath -GameInfo $script:ConfigState.SelectedGame
            if ($dgVoodooResult.Success) {
                Write-Status -Message "dgVoodoo2 installed for graphics compatibility" -Type "Success"
                Write-Host "    - DirectDraw/Direct3D to DirectX 11 translation" -ForegroundColor Gray
                Write-Host "    - Simulates older graphics hardware" -ForegroundColor Gray
                Write-Host "    - Fixes vehicle movement issues" -ForegroundColor Gray
            ...
            try {
                ...
                Write-Host "    - GDI renderer for compatibility" -ForegroundColor Gray
        ...
        # Standard cnc-ddraw installation for Army Men and Toys in Space
    ...
    try {
        $configResult = Set-GameResolution -GamePath $script:ConfigState.GamePath -Width $script:ConfigState.Resolution.Width -Height $script:ConfigState.Resolution.Height -GameInfo $script:ConfigState.SelectedGame
